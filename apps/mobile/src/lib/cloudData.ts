...
async function shouldGenerateOccurrence(template: TaskTemplate, date: string, ownerId: string): Promise<boolean> {
  ...
  if (template.custom_days) {
    ...
    if (customDays.includes(day)) return true;
  }
...
export async function cloudCreateCategory(data: Pick<Category, 'name' | 'icon_path' | 'color_hex'>): Promise<Category> {
  const ownerId = await requireUserId();
  const { data: row, error } = await supabase!.from('categories').insert({ ...data, owner_id: ownerId, is_system: false }).select('*').single();
  if (error) throw error;
...
export async function cloudUpdateCategory(id: string, data: Partial<Pick<Category, 'name' | 'icon_path' | 'color_hex'>>): Promise<Category> {
  const ownerId = await requireUserId();
  const { data: row, error } = await supabase!.from('categories').update(data).eq('id', id).eq('owner_id', ownerId).eq('is_system', false).select('*').single();
  if (error) throw error;
  return row as Category;
}
...
export async function cloudDeleteCategory(id: string): Promise<void> {
  const ownerId = await requireUserId();
  const { error } = await supabase!.from('categories').update({ deleted_at: nowIso() }).eq('id', id).eq('owner_id', ownerId).eq('is_system', false);
  if (error) throw error;
}

export async function cloudGetTaskTemplates(): Promise<TaskTemplate[]> {
  const ownerId = await requireUserId();
  const { data, error } = await supabase!.from('task_templates').select('*').eq('owner_id', ownerId).is('deleted_at', null).order('created_at', { ascending: false });
  if (error) throw error;
...
export async function cloudCreateTaskTemplate(data: Omit<TaskTemplate, 'id' | 'owner_id' | 'created_at' | 'updated_at' | 'deleted_at' | 'sync_version'>): Promise<TaskTemplate> {
  const ownerId = await requireUserId();
  const { data: row, error } = await supabase!.from('task_templates').insert({ ...data, owner_id: ownerId }).select('*').single();
  if (error) throw error;
...
export async function cloudUpdateTaskTemplate(id: string, data: Partial<Omit<TaskTemplate, 'id' | 'owner_id' | 'created_at' | 'updated_at' | 'deleted_at' | 'sync_version'>>): Promise<TaskTemplate> {
  const ownerId = await requireUserId();
  const { data: row, error } = await supabase!.from('task_templates').update(data).eq('id', id).eq('owner_id', ownerId).select('*').single();
  if (error) throw error;
  return row as TaskTemplate;
}
...
export async function cloudDeleteTaskTemplate(id: string): Promise<void> {
  const ownerId = await requireUserId();
  const { error } = await supabase!.from('task_templates').update({ deleted_at: nowIso() }).eq('id', id).eq('owner_id', ownerId);
  if (error) throw error;
}
...
export async function cloudGetTaskOccurrences(params: GetOccurrencesParams = {}): Promise<TaskOccurrence[]> {
  const ownerId = await requireUserId();
...
export async function cloudCreateTaskOccurrence(data: any): Promise<TaskOccurrence> {
  const ownerId = await requireUserId();
  const { data: row, error } = await supabase!.from('task_occurrences').insert({ ...data, owner_id: ownerId, task_template_id: data.task_template_id ?? null, is_detached: false }).select('*').single();
  if (error) throw error;
  return row as TaskOccurrence;
}

export async function cloudUpdateTaskOccurrence(id: string, data: any): Promise<TaskOccurrence> {
  const ownerId = await requireUserId();
...
export async function cloudCheckConflict(data: { date: string; start_time: string; time_to_complete: number; exclude_id?: string; }): Promise<ConflictCheck> {
  const ownerId = await requireUserId();
  const { data: rows, error } = await supabase!.from('task_occurrences').select('*').eq('owner_id', ownerId).eq('date', data.date).neq('status', 'COMPLETED').is('deleted_at', null).order('start_time', { ascending: true });
  if (error) throw error;
  ...
  return { has_conflict: conflicting.length > 0, conflicting_tasks: conflicting.map(rowToConflictTask) };
}
...
export async function cloudPlayTimer(task_occurrence_id: string): Promise<TimerSession> {
  ...
  const { data: row, error } = await supabase!.from('timer_sessions').insert({ owner_id: ownerId, task_occurrence_id, start_time: nowIso(), is_active: true }).select('*').single();
  if (error) throw error;
...
export async function cloudGetTimeSpent(start_date: string, end_date: string): Promise<TimeSpentEntry[]> {
  const ownerId = await requireUserId();
  ...
  return [...grouped.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([date, total_seconds]) => ({ date, total_seconds }));
}

export async function cloudGetCompletionRate(start_date: string, end_date: string): Promise<CompletionRateEntry[]> {
  const ownerId = await requireUserId();
  ...
  return [...grouped.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([date, stats]) => ({ date, completed: stats.completed, total: stats.total, rate: stats.total > 0 ? Math.round((stats.completed / stats.total) * 10000) / 10000 : 0 }));
}
