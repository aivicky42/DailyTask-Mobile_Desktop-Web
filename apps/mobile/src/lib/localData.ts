...
export async function localGetTimeSpent(start_date: string, end_date: string): Promise<TimeSpentEntry[]> {
...
export async function localGetCompletionRate(start_date: string, end_date: string): Promise<CompletionRateEntry[]> {
  const rows = await localGetTaskOccurrences({ start_date, end_date });
  ...
  return [...grouped.entries()]
    ...
    .map(([date, stats]) => ({
      ...
      rate: stats.total > 0 ? Math.round((stats.completed / stats.total) * 10000) / 10000 : 0,
    }));
}
